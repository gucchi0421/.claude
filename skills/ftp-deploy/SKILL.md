...
description: >
  ...
  ~/.claude/scripts/deploy_ftp.py を使い、プロジェクト直下の .env から接続情報を読む。
---
...
```bash
python3 ~/.claude/scripts/deploy_ftp.py --test
```
...
```bash
python3 ~/.claude/scripts/deploy_ftp.py
```
...
- FTP操作のためのインラインスクリプトを書かない。必ず `python3 ~/.claude/scripts/deploy_ftp.py` を使う
...
| 接続失敗（FTPS 非対応） | サーバーが FTPS 未対応の可能性。`deploy_ftp.py` の `FTP_TLS` を `FTP` に変更するか確認する |
