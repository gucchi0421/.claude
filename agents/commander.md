...
SEO記事パイプラインの正しい手順：
1. commander が `mcp__gsc__*` と `mcp__analytics-mcp__*` でデータ取得・分析まで行う（data-analyst は使わない）
2. 分析結果を渡して `competitor-analyst` を起動（競合コンテンツ調査）
3. メインの分析結果 + competitor-analyst の結果を渡して `seo-analyst` を起動（キーワード選定・執筆指示書）
